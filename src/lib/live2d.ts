...
import { Live2DSoundPauseCoordinator } from './Live2DSoundPauseCoordinator'
...
const soundPauseCoordinator: Live2DSoundPauseCoordinator = new Live2DSoundPauseCoordinator(
  (): unknown => live2DEngine?.SoundManager ?? null
)
...
export function pauseSekaiLive2DSounds(owner: object): void {
  soundPauseCoordinator.pause(owner)
}

export function resumeSekaiLive2DSounds(owner: object): void {
  soundPauseCoordinator.resume(owner)
}

/** Pauses sounds that finished loading after story playback was already paused. */
export function synchronizePausedSekaiLive2DSounds(): void {
  soundPauseCoordinator.synchronize()
}
